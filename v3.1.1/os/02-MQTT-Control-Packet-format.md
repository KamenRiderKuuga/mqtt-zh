...
`Remaining Length` 表示当前报文中剩余的字节数，包括可变报头和有效载荷中的数据。`Remaining Length` 不包括用于编码剩余长度本身的字节数。

`Remaining Length` 使用变长编码方案进行编码，对于不超过 127 的值使用单个字节。较大的值按以下方式处理：每个字节的低 7 位编码数据，最高位用于指示表示中是否还有后续字节。因此，每个字节编码 128 个值和一个"继续位"。`Remaining Length` 字段的最大字节数为 4。
...
表 2.4 显示了由递增字节数表示的 `Remaining Length` 值。

**表 2.4 `Remaining Length` 字段的大小**
...
`Client` 和 `Server` 独立分配 `Packet Identifier`。因此，`Client`-`Server` 对可以使用相同的 `Packet Identifier` 参与并发的消息交换。

> **非规范性说明**
>
> `Client` 可能会发送一个 `Packet Identifier` 为 0x1234 的 `PUBLISH` 报文，然后在收到其发送的 `PUBLISH` 的 `PUBACK` 之前，从 `Server` 接收到另一个 `Packet Identifier` 为 0x1234 的 `PUBLISH`。
>
> ```
>   Client                      Server
