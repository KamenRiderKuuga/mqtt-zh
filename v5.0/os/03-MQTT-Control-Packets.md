...
发送 SUBACK 报文的 Server 必须为收到的每个 Topic Filter 使用其中一个 Subscribe Reason Codes [MQTT-3.9.3-2]。
...
UNSUBSCRIBE 报文的 Payload 包含 Client 希望取消订阅的 Topic Filter 列表。UNSUBSCRIBE 报文中的 Topic Filter 必须是 UTF-8 Encoded Strings [MQTT-3.10.3-1]，如 [第 1.5.4 节](#154-utf-8-encoded-string)所定义，连续打包。
...
**表 3-9 - Unsubscribe Reason Codes**
...
发送 UNSUBACK 报文的 Server 必须为收到的每个 Topic Filter 使用其中一个 Unsubscribe Reason Codes [MQTT-3.11.3-2]。
...
Variable Header 中的字节 0 是 Authenticate Reason Code。单字节无符号 Authenticate Reason Code 字段的值如下所示。AUTH 报文的发送方必须使用其中一个 Authenticate Reason Codes [MQTT-3.15.2-1]。

**表 3-11 - Authenticate Reason Codes**
