...
## 4.7 Topic Names and Topic Filters `Topic Name` 和 `Topic Filter`
...
- 0x9E `Shared Subscription` not supported 不支持 `Shared Subscription`
- 0xA1 `Subscription Identifier` not supported 不支持 `Subscription Identifier`
- 0xA2 Wildcard `Subscription` not supported 不支持通配符 `Subscription`
